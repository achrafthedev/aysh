                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Aysh (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Aysh" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "aysh_kind" if "aysh_kind" in cols else "'scheduled' AS aysh_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                aysh_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Aysh-Origin"] = "aysh-ui"
                outer["X-Aysh-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", aysh_kind or "scheduled")[:64]
                outer["X-Aysh-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `aysh-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `AYSH_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `aysh-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("AYSH_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `AYSH_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (AYSH_INPROCESS_POLLERS=0); "
            "drive `aysh-mail poll-scheduled` externally."
        )
        return
    import asyncio
