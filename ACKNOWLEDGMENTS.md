...
**Aysh is a fork of [Odysseus](https://github.com/pewdiepie-archdaemon/odysseus)**,
the self-hosted AI workspace originally released by PewDiePie
(pewdiepie-archdaemon). Aysh keeps Odysseus's architecture, feature set, and
AGPL-3.0-or-later license, and layers on a rebrand plus a new always-listening
wake-word voice assistant (see the [README](README.md#-voice-assistant----say-aysh)).
All credit for the original design and the vast majority of the code goes to
the upstream Odysseus project and its contributors — please go star it.

Aysh stands on the shoulders of a lot of open-source work. This file
...
- **[Tongyi DeepResearch](https://github.com/Alibaba-NLP/DeepResearch)** by
  **Alibaba-NLP / Tongyi Lab** — the multi-step deep-research agent pipeline.
  Copyright © Alibaba-NLP / Tongyi Lab. **Apache-2.0.** Adapted for Aysh's
...
and run alongside Aysh on `docker compose up`. They are not modified —
just composed.
...
Aysh talks to these over the network/API. They are **not** distributed
...
- **`caldav`** (Python lib) is **dual-licensed GPL-3.0-or-later OR Apache-2.0**.
  Aysh uses it under **Apache-2.0**, which is permissive and MIT-compatible.
...
Most of Aysh's code was written *with* AI models, not just by a human.
