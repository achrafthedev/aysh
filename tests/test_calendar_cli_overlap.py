"""Regression: `aysh-calendar list` must select events that OVERLAP the
...
`aysh-calendar list` with the default start=now()).
"""
...
def _load_cli(monkeypatch, rows):
    ...
    path = ROOT / "scripts" / "aysh-calendar"
    loader = importlib.machinery.SourceFileLoader("aysh_calendar_cli", str(path))
