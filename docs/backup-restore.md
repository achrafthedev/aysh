...
Aysh keeps all of your state in the `data/` directory — the SQLite database
(`app.db`), the Fernet encryption key (`data/.app_key`), the vault, memory, RAG
indexes, personal documents, and uploads. The `scripts/aysh-backup` tool
...
```bash
# Create a snapshot → backups/aysh-backup-<YYYYMMDD-HHMMSS>.tar.gz
./scripts/aysh-backup snapshot

# List existing snapshots (most recent first)
./scripts/aysh-backup list

# Check a tarball's integrity without extracting it
./scripts/aysh-backup verify backups/aysh-backup-20260101-120000.tar.gz

# Restore (destructive — see the warning below)
./scripts/aysh-backup restore backups/aysh-backup-20260101-120000.tar.gz --yes
```
...
./scripts/aysh-backup snapshot --out /mnt/nas/aysh-$(date +%F).tar.gz

# Full snapshot including research runs and mail attachments
./scripts/aysh-backup snapshot --include-research --include-attachments
```
...
```cron
0 3 * * *  cd /path/to/aysh && ./scripts/aysh-backup snapshot --out "/mnt/nas/aysh-$(date +\%F).tar.gz"
```
...
> **not** under `./data`. `aysh-backup` therefore does not capture the Docker
